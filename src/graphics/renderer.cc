...
void Renderer::init(SDL_GPUCommandBuffer* cmd)
{
	...
	SDL_GPUBufferCreateInfo inst_info = {
		...
		.size = static_cast<uint32_t>(max_particles_ * sizeof(ParticleInstance)),
	};
	instance_vb_ = SDL_CreateGPUBuffer(gpu_context_->get_device(), &inst_info);
}

void Renderer::update(SDL_GPUCommandBuffer* cmd, std::shared_ptr<ParticleSystem> particle_system)
{
	...
	int W, H; SDL_GetWindowSizeInPixels(gpu_context_->get_window(), &W, &H);
	float mx, my;
...
void Renderer::render(SDL_GPUCommandBuffer* cmd_buf)
{
	...
	// VS ortho for particles (set=1, binding=0)
	int W, H;
	SDL_GetWindowSizeInPixels(gpu_context_->get_window(), &W, &H);
	// glm::ortho(left, right, bottom, top, zNear, zFar)
	glm::mat4 proj = glm::ortho(0.0f, (float)W, (float)H, 0.0f, -1.0f, 1.0f);
	struct OrthoUBO { glm::mat4 proj; } ubo{proj};
	SDL_PushGPUVertexUniformData(cmd_buf, /*slot*/0, &ubo, sizeof(ubo)); // -> set=1,binding=0
	...
	// triangle pass
	{
		...
		ctx.clear = true;
		ctx.clear_color = {0.f, 0.f, 240.f/255.f, 1.f};
                                     ...
                                     uint32_t    num_UBOs)
{
	...
	if (!shader) {
		...
		SDL_free(metadata);
		SDL_free(bytes);
		throw std::runtime_error(std::string("Shader compile failed: ") + path + " :: " + err);
	}
...
void Renderer::create_particle_pipeline()
{
	...
	SDL_GPUVertexAttribute attrs[] = {
		...
		{ .buffer_slot=0, .location=1, .format=SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2, .offset=sizeof(float)*2 },

		// slot 1 (instance) — POZOR: offsetof!
		{ .buffer_slot=1, .location=2, .format=SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2,
		  .offset=(uint32_t)offsetof(ParticleInstance, world_pos) },

		{ .buffer_slot=1, .location=3, .format=SDL_GPU_VERTEXELEMENTFORMAT_FLOAT,
		  .offset=(uint32_t)offsetof(ParticleInstance, size) },

		{ .buffer_slot=1, .location=4, .format=SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4,
	...
	particle_pipeline_ = SDL_CreateGPUGraphicsPipeline(gpu_context_->get_device(), &ci);
}
