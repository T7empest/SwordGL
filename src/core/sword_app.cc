...
bool SwordApp::handle_event(const SDL_Event &event)
{
    ...
    if (event.type == SDL_EVENT_MOUSE_MOTION)
    {
        ...
        for (int i{0}; i < utils::randomFloat(10.0f, 100.0f); ++i) particle_system_->emit(dt_, mouse_pos);
    }
...
{
#ifdef __APPLE__
    window_ = SDL_CreateWindow(window_title.data(), window_start_width,
                               window_start_height,
                               SDL_WINDOW_RESIZABLE | SDL_WINDOW_METAL);
#else
    window_ = SDL_CreateWindow(window_title.data(), window_start_width,
                               window_start_height, SDL_WINDOW_RESIZABLE);
#endif
